...
(function()
{
	...
	Runtime.prototype.waitForImageLoad = function (img_, src_)
	{
		...
		if (window.cr_remotePreviewImageFiles && src_ && src_.substr(0, 5) !== "blob:")		// note ignore if already a blob URL (may have come from getLocalFileUrl())
		{
			...
			assert2(src_, "missing remote preview image");
		}
	...
	Runtime.prototype.logic = function (cur_time)
	{
		...
		if (cur_time - this.last_fps_time >= 1000)  // every 1 second
		{
			...
			if (this.isDebug)
			{
				...
				DebuggerPerfStats(this.fps, this.cpuutilisation, this.gpuLastUtilisation, this.glwrap ? this.glwrap.estimateVRAM() : -1, this.glwrap ? "webgl" : "canvas2d", this.objectcount, this.rendercpu, this.eventscpu, this.physics_cpu, sheets_perf);
			}
		...
		if (type.is_contained)
		{
			...
			if (!is_startup_instance && !skip_siblings)	// layout links initial instances
			{
				for (i = 0, len = type.container.length; i < len; i++)
				{
					...
					{
						/**PREVIEWONLY**/ alert("Cannot create an instance of the object type '" + type.container[i].name + "': there are no instances of this object anywhere in the project.  Construct 3 needs at least one instance to know which properties to assign to the object.  To resolve this, add at least one instance of the object to the project, on an unused layout if necessary.");
						return null;
					}
	...
	Runtime.prototype.debugBreak = function ()
	{
		...
		// resume and break happen in quick sequence) there is a possible race between the breakpoint off message
		// from the previous event, and this breakpoint on message. If this message arrives first the previous
		// event's breakpoint off message will undo its effect. So this message is slightly delayed to try to ensure
		// it is the second one to arrive.
		var data = {
			"eventSheet": this.breakpoint_event.sheet.name,
			"eventNumber": this.breakpoint_event.display_number,
			"condition": (this.breakpoint_condition ? this.breakpoint_condition.index : -1),
			"action": (this.breakpoint_action ? this.breakpoint_action.index : -1)
		};
		
		setTimeout(function ()
		{
			DebuggerOnHitBreakpoint(data);
		}, 50);
	};
