...
cr.system_object.prototype.getDebuggerValues = function (propsections)
{
	...
	propsections.push({
		...
		"properties": [
			{"name": ["system.performance.fps"], "value": ["system.performance.fps-value", runtime.fps, (Math.round(10000 / runtime.fps) / 10)], "readonly": true},
			{"name": ["system.performance.cpu"], "value": "" + (Math.round(runtime.cpuutilisation) / 10) + "%", "readonly": true},
			{"name": ["system.performance.image-mem"], "value": runtime.glwrap ? ["system.performance.image-mem-value", Math.round(10 * runtime.glwrap.estimateVRAM() / (1024 * 1024)) / 10] : ["system.performance.image-mem-unavailable"], "readonly": true},
			{"name": ["system.performance.renderer"], "value": runtime.glwrap ? "webgl" : "canvas2d", "readonly": true},
			{"name": ["system.performance.object-count"], "value": runtime.objectcount, "readonly": true},
			{"name": ["system.performance.collision-checks"], "value": ["system.performance.value-per-tick", runtime.collisioncheck_sec, Math.round(runtime.collisioncheck_sec / runtime.fps)], "readonly": true},
			{"name": ["system.performance.poly-checks"], "value": ["system.performance.value-per-tick", runtime.polycheck_sec, Math.round(runtime.polycheck_sec / runtime.fps)], "readonly": true},
			{"name": ["system.performance.moved-cell"], "value": ["system.performance.value-per-tick", runtime.movedcell_sec, Math.round(runtime.movedcell_sec / runtime.fps)], "readonly": true},
			{"name": ["system.performance.cell-count"], "value": "" + cr.SparseGrid.prototype.totalCellCount, "readonly": true},
			{"name": ["system.performance.moved-render-cell"], "value": ["system.performance.value-per-tick", runtime.movedrendercell_sec, Math.round(runtime.movedrendercell_sec / runtime.fps)], "readonly": true},
			{"name": ["system.performance.render-cell-count"], "value": "" + cr.RenderGrid.prototype.totalCellCount, "readonly": true}
		]
	});
	...
	propsections.push({
		...
		"properties": [
			{"name": ["system.system.canvas-size"], "value": "(" + runtime.width + "," + runtime.height + ")", "readonly": true},
			{"name": ["system.system.time-scale"], "value": runtime.timescale},
			{"name": ["system.system.time"], "value": runtime.kahanTime.sum, "readonly": true},
			{"name": ["system.system.wallclock-time"], "value": (Date.now() - this.runtime.start_time) / 1000.0, "readonly": true},
			{"name": ["system.system.tick-count"], "value": runtime.tickcount, "readonly": true}
		]
	});
	...
	{
		propsections.push({
			"title": ["system.static-local-variables"],
			"properties": props
		});
	}
	...
	propsections.push({
		...
		"properties": [
			{"name": ["system.layout.name"], "value": layout.name, "readonly": true},
			{"name": ["system.layout.width"], "value": layout.width},
			{"name": ["system.layout.height"], "value": layout.height},
			{"name": ["system.layout.event-sheet"], "value": layout.event_sheet ? layout.event_sheet.name : "(none)", "readonly": true},
			{"name": ["system.layout.scroll-x"], "value": layout.scrollX},
			{"name": ["system.layout.scroll-y"], "value": layout.scrollY},
			{"name": ["system.layout.scale"], "value": layout.scale},
			{"name": ["system.layout.angle"], "value": cr.to_degrees(layout.angle)}
		]
	});
	...
	for (i = 0, len = layout.layers.length; i < len; ++i)
	{
		...
		props.push({"name": ["system.layer.index"], "value": layer.index, "readonly": true});
		props.push({"name": ["system.layer.scale"], "value": layer.scale});
		props.push({"name": ["system.layer.angle"], "value": cr.to_degrees(layer.angle)});
		props.push({"name": ["system.layer.viewport-left"], "value": layer.viewLeft, "readonly": true});
		props.push({"name": ["system.layer.viewport-top"], "value": layer.viewTop, "readonly": true});
		props.push({"name": ["system.layer.viewport-right"], "value": layer.viewRight, "readonly": true});
		props.push({"name": ["system.layer.viewport-bottom"], "value": layer.viewBottom, "readonly": true});
		props.push({"name": ["system.layer.visible"], "value": layer.visible});
		props.push({"name": ["system.layer.transparent"], "value": layer.transparent});
		props.push({"name": ["system.layer.parallax-x"], "value": layer.parallaxX * 100});
		props.push({"name": ["system.layer.parallax-y"], "value": layer.parallaxY * 100});
		props.push({"name": ["system.layer.opacity"], "value": layer.opacity * 100});
		props.push({"name": ["system.layer.force-own-texture"], "value": layer.forceOwnTexture});
		props.push({"name": ["system.layer.zoom-rate"], "value": layer.zoomRate});
		
		propsections.push({
			"title": ["system.layer.title", layer.name],
			"properties": props
		});
	}
};
...
cr.system_object.prototype.onDebugValueEdited = function (header, name, value)
{
	...
	if (header === "system.system.title")
	{
		if (name === "system.system.time-scale")
			runtime.timescale = value;
	}
	else if (header === "system.global-variables")
	{
	...
	else if (header === "system.static-local-variables")
	{
	...
	else if (header === "system.layout.title")
	{
		switch (name) {
		case "system.layout.width":			layout.width = value;				break;
		case "system.layout.height":		layout.height = value;				break;
		case "system.layout.scroll-x":		layout.scrollToX(value);			break;
		case "system.layout.scroll-y":		layout.scrollToY(value);			break;
		case "system.layout.scale":			layout.scale = value;				break;
		case "system.layout.angle":			layout.angle = cr.to_radians(value); break;
		}
	}
	else {
		// Parse layer name from string "system.layer.title,LayerName"
		var layername = header.split(",")[1];
		var layer = null;
		...
		{
			switch (name) {
			case "system.layer.scale":					layer.scale = value;				break;
			case "system.layer.angle":					layer.angle = cr.to_radians(value);	break;
			case "system.layer.visible":				layer.visible = value;				break;
			case "system.layer.transparent":			layer.transparent = value;			break;
			case "system.layer.parallax-x":				layer.parallaxX = value / 100;		break;
			case "system.layer.parallax-y":				layer.parallaxY = value / 100;		break;
			case "system.layer.opacity":				layer.opacity = value / 100;		break;
			case "system.layer.force-own-texture":		layer.forceOwnTexture = value;		break;
			case "system.layer.zoom-rate":				layer.zoomRate = value;				break;
			}
		}
	}
};
...
(function ()
{
	...
	{
		assert2(!v.is_constant, "Setting a constant event variable");
		assert2(v.vartype === 2, "expected boolean type");
